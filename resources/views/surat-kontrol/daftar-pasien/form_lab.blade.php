        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 row">
            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                ...
                <div class="form-group">
                    ...
                    <div class="input-group col-sm-9">
                        <span class="input-group-addon" id="" style="border:none;background-color:transparent;">:</span>
                        <input type="text" name="Medrec" id="Medrec" value="{{ @$medrec }}"/>
                ...
                <div class="form-group">
                    ...
                    <div class="input-group col-sm-9">
                        <span class="input-group-addon" id="" style="border:none;background-color:white;">:</span>
                        <input type="text" name="NoSuratKontrol" class="form-control input-sm" id="NoSuratKontrol" value="{{ @$nosurat }}"/>
                        <!-- Tanggal Registrasi -->
...
<script>
    ...
    $('#search_nosurat').submit(function(ev) {
        ...
        loading.modal('show');
        $.ajax({
            url:"{{ route('reg-bpjs-daftar.find-rujuklab') }}",
            type:"get",
            dataType:"json",
            data:{
            ...
            success:function(response){
                console.log(response);
                $('#RegRujuk').val(response.RegDate.substring(0,10));
                ...
                $('#poli').append($poli).trigger('change');
                var $diagnosa = $("<option selected></option>").val(response.KdICD).text(response.Diagnosa);
                            $('#Diagnosa').append($diagnosa).trigger('change');
    ...
    $('#search_nosurat2').submit(function(ev) {
        ev.preventDefault();
        let loading = $('.modal-loading');
        loading.modal('show');
        $.ajax({
                                        ...
                                        if(resp.status){
                                            ...
                                            $.ajax({
                                                ...
                                                success:function(response)
                                                {
                                                    ...
                                                    update_laboratorium('<?=$nosurat?>', response.data.Regno);
                                            ...
                                            update_laboratorium('<?=$nosurat?>', resp.data.Regno);
                                            loading.modal('hide');
    ...
    function update_laboratorium(notrans, regno) {
        $.ajax({
            url:"{{ route('reg-bpjs-daftar.up-laboratorium') }}",
            type:"get",
            dataType:"JSON",
            data:{
                notrans: notrans, regno:regno
            },
            success: function(response)
            {
                console.log(response);
