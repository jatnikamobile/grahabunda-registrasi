...
class VClaim
{
	...
	public static $namaPpkPelayanan = 'RSUD Raja Ahmad Tabib';
	...
	{
		$url = config('vclaim.url').'referensi/faskes/'.urlencode($nama).'/'.urlencode($jenis);
		$response = self::send_curl($url);
		return $response ? json_decode($response) : null;
	}
	 ...
	 * @return object
	 */
	public static function get_peserta_bpjs($no_kartu, $tanggal_sep)
	{
		$url = config('vclaim.url').'Peserta/nokartu/'.urlencode($no_kartu).'/tglSEP/'.urlencode($tanggal_sep);
		$response = self::send_curl($url);
		return $response ? json_decode($response) : null;
	}
	...
	{
		$url = config('vclaim.url').'Peserta/nik/'.urlencode($nik).'/tglSEP/'.urlencode($tanggal_sep);
		$response = self::send_curl($url);
		return $response ? json_decode($response) : null;
	}
	...
	{
		$url = config('vclaim.url').'SEP/'.urlencode($no_sep);
		$response = self::send_curl($url);
		return $response ? json_decode($response) : null;
	}
	...
	{
		$url = config('vclaim.url').'Rujukan/'.($rs ? 'RS/' : '').$no_rujukan;
	...
	{
		$url = config('vclaim.url').'Rujukan/'.($rs ? 'RS/' : '').'Peserta/'.$no_kartu;
	...
	{
		$url = config('vclaim.url').'Rujukan/'.($rs ? 'RS/' : '').'List/Peserta/'.$no_kartu;
	...
	{
		$url  = config('vclaim.url').'monitoring/HistoriPelayanan/NoKartu/'.urlencode($no_kartu);
	...
	private static function send_curl($url, $method = 'GET', $data = NULL) {
		...
		if ($_REQUEST['__debug'] ?? 0) {
			$debugFile = fopen(str_replace('\\','/', __DIR__).'/../../storage/logs/curl-debug.log', 'a+');
